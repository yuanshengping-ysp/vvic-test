create table if not exists `ods`.`vvic__t_shop_vas_order` ( 
...
`order_cycle_start` string comment '生效时间',
`order_cycle_end` string comment '失效时间',
`order_fee` double comment '订单价格-应付',
`prom_fee` double comment '订单优惠',
`total_pay_fee` double comment '实付',
`prom_desc` string comment '优惠说明',
...
`recommend_shop` string comment '推荐档口',
...
`owner_user_name` string comment '档口负责人姓名' ) 
 stored as textfile 
 location 'hdfs://master:8020/user/hive/warehouse/ods.db/vvic__t_shop_vas_order' ;